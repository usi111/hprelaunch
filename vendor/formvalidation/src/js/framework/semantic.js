		...
		FormValidation.Framework.Semantic = function (element, options)
			{
				options = $.extend(true, {
					button: {
						selector: '[type="submit"]:not([formnovalidate])', // CSS class of disabled button
						...
						clazz: 'ui red pointing label transition', parent: '^.*(field|column).*$'
					...
					icon: {
						...
						selector: '.field', valid: 'fv-has-success', invalid: 'error', feedback: 'fv-has-feedback'
					}
				}, options);
							...
							switch (type)
							{
								case 'popover':
									$icon.css({
											...
											content: message, position: 'top center'
										});
									break;
								...
								default:
									$icon.css({
											...
											content: message, position: 'top center', variation: 'inverted'
										});
									break;
							}
						}
					...
					if ($icon && $icon.popup('exists'))
						{
							$icon.css({
									'cursor': ''
								}).popup('remove popup').popup('destroy');
						}
				},
